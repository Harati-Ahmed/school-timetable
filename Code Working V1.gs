...
function updateSummary() {
  ...
  // Write teacher summary data
  if (teacherSummaryData.length > 0) {
    // Clear existing data except SI column
    summarySheet.getRange(3, 2, 26, 2).clearContent(); // Clear only columns 2-3
    
    // Write new data (skip SI column), limit to 26 rows
    ...
    // If no teacher data, clear the teacher summary area except SI column
    summarySheet.getRange(3, 2, 26, 2).clearContent(); // Clear only columns 2-3
  }
  
  // Process class-subject assignments
    ...
    for (let i = 1; i < row.length; i++) {
      if (row[i] && i !== BREAK_COL - 2 && i !== LUNCH_COL - 2) {
        ...
        if (!classSubjectMap.has(className)) {
          classSubjectMap.set(className, new Map());
        }
        classSubjectMap.get(className).set(subject, teacher);
      }
    }
  });
  
  // Create class-subject matrix
  const subjectTeachersData = classes.map(className => {
    const subjectMap = classSubjectMap.get(className) || new Map();
    return subjects.map(subject => subjectMap.get(subject) || '');
  });
  
  // Write class-subject data (preserve SI and class names)
  if (subjectTeachersData.length > 0) {
    // Clear only the subject columns
    summarySheet.getRange(3, 7, 21, subjects.length).clearContent();
    
    // Write new subject data, limit to 21 rows
    const rowsToWrite = Math.min(subjectTeachersData.length, 21);
    summarySheet.getRange(3, 7, rowsToWrite, subjects.length)
      .setValues(subjectTeachersData.slice(0, rowsToWrite));
  }
  ...
  const rightTable = summarySheet.getRange(3, 5, 21, 8); // SI + Class + 6 subjects
